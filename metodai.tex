...
Neuroninis tinklas sudarytas iš daugelio neuronų. Kiekvienas neuronas gauna kelias įvestis, kiekviena iš jurių turi savo svorį. Iš pradžių svorio reikšmės yra parenkamos atsitiktiniu būdu, tačiau keičiamos modeliui besimokant. Neurono produkto skaičiavimo formulė atrodo taip:
\begin{equation}
...
% galimai formules reikės pernešt į literatūrinę dalį??
...
y = x\cdot W^{T} + b
\end{equation}
kur $W$ yra svorių vektorius $\{w_1, w_2, ..., w_k\}$, $T$ yra \TD{???}

Po tiesinio transformavimo vektoriaus matmenys keičiasi  iki 'cf' dydžio. \TD{kas yra cf?} Tuomet yra taikoma ReLU funkcija. Tai yra netiesinė aktyvacijos funkcija: ji apdoroja gautą vektorių ir grąžina jo tikslią vertę, jei ji yra didesnė už nulį, priešingu atveju grąžina nulį. Ji dažniausiai yra naudojama konvoliuciniams neuroniniams tinklams, o jos formulė yra:
\begin{equation}
 g(z) = max\{0, z\}
\end{equation}
